...
```{r pheno}
pheno <- read_tsv('/exports/eddie/scratch/v1ielli3/ComplexCVDTraits/users/ielliott/Week10/pheno.tsv')
pheno
```
...
```{r geno}
genotypes_chr11 <- read_tsv("/exports/eddie/scratch/v1ielli3/ComplexCVDTraits/users/ielliott/Week10/genotypes.11-113200000-113500000.tsv.gz")
genotypes_chr20 <- read_tsv("/exports/eddie/scratch/v1ielli3/ComplexCVDTraits/users/ielliott/Week10/genotypes.20-34000000-34100000.tsv.gz")
```
...
```{r freq}
freq_chr11 <- read_tsv("/exports/eddie/scratch/v1ielli3/ComplexCVDTraits/users/ielliott/Week10/genotypes.11-113200000-113500000.afreq")
freq_chr20 <- read_tsv("/exports/eddie/scratch/v1ielli3/ComplexCVDTraits/users/ielliott/Week10/genotypes.20-34000000-34100000.afreq")
```
...
Merge the phenotype and genotype data together. 
Make two data frames, one called `pheno_chr11` that has the 
phenotypes merged with the genotypes from chromosome 11 and
 another called `pheno_chr20` with the genotypes from chromosome 20.

 pheno_chr11 <- merge(pheno, genotypes_chr11)
pheno_chr20 <- merge(pheno, genotypes_chr20)
## GWAS as regression
...
A Manhattan plot is a standard way of visually presenting GWAS results. It compares SNPs by their basepair position along the X axis to $-log_{10}(P)$ on the Y axis.
 ...
 height_sumstats_chr20 %>% ggplot(aes(x= POS, y= -log10(P)))+ geom_point()+
 labs(title=  "Chromsome 20 SNPs and height", 
x= "Position on Chromosome 20", 
y= "-log10(P Value)")+
...
## Exercise 9: Real GWAS summary statistics

Download a set of summary statistics from a real study and open them in R. Possible sources include:

-   [Psychiatric Genomics Consortium](https://pgc.unc.edu/for-researchers/download-results/)
-   [Centre for Cognitive Aging and Cognitive Epidemiology](https://www.ccace.ed.ac.uk/node/335)
...
Try using the [ggman](https://github.com/drveera/ggman) package to make a Manhattan plot.
...
plot1 <- height_sumstats_chr20 %>% ggplot(aes(x= POS, y= -log10(P)))+ geom_point()+
 labs(title=  "Chromsome 20 SNPs and height", 
x= "Position on Chromosome 20", 
y= "-log10(P Value)")+
 geom_hline(yintercept = -log10(5e-8), col='red')
 
ggsave("/exports/eddie/scratch/v1ielli3/ComplexCVDTraits/users/ielliott/Week10/plot1.png",plot=plot1)
